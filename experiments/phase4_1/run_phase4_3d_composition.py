"""ANTLER Phase 4.3d: direct composition of sequential digital exchange cycles.
...
class DConfig:
 ...
 DEPTH:float=-4.;R_LOOP:int=4;T_TOTAL:float=20000.;CYCLES:int=2
...
def mu_digital(u,exchange,cfg):
 ...
 mu[-2]=D;mu[-1]=D
 # 0-.35 outbound; .35-.45 stationary left swap; .45-.55 mobile rung swap;
 # .55-1 return. The two avoided crossings are never simultaneous.
 if u<.35:
  x=R*(u/.35);add_discrete_trap(mu,0,x,D,cfg.L);mu[1]+=D
 elif u<.45:
  s=(u-.35)/.10
  mu[2*R]+=D
  if exchange:
   q=sin2(s);mu[1]+=D*(1-q);mu[0]+=D*q
  else:mu[1]+=D
 elif u<.55:
  s=(u-.45)/.10
  if exchange:
   q=sin2(s);mu[2*R]+=D*(1-q);mu[2*R+1]+=D*q;mu[0]+=D
  else:mu[2*R]+=D;mu[1]+=D
 else:
  x=R*(1-(u-.55)/.45)
  if exchange:add_discrete_trap(mu,1,x,D,cfg.L);mu[0]+=D
  else:add_discrete_trap(mu,0,x,D,cfg.L);mu[1]+=D
 return mu
...
def main():
 ...
 ap.add_argument('--cycles',type=int,default=2);ap.add_argument('--out',type=Path,required=True);args=ap.parse_args();cfg=DConfig(DEPTH=args.depth,R_LOOP=args.R,T_TOTAL=args.T,CYCLES=args.cycles)
 ...
 rel=float(np.angle(np.exp(1j*(np.angle(Uodd[0,0])-np.angle(Uodd[1,1])))));target=np.diag([np.exp(-1j*cfg.CYCLES*args.theta/2),np.exp(1j*cfg.CYCLES*args.theta/2)])
 metrics={'sigma_min':float(min(svs)),'sigma_max':float(max(svs)),'leak_worst':float(1-min(svs)**2),'unitarity_frob_max':float(max(units)),'odd_phase':rel,'odd_slope_per_cycle':rel/(cfg.CYCLES*args.theta),'odd_offdiag_norm':float(np.linalg.norm(Uodd-np.diag(np.diag(Uodd)))),'favg_target':favg(Uodd,target)}
