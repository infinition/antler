...
def logical_frame(H0, index: dict[int, int], M: int) -> tuple[np.ndarray, dict]:
    ...
    # Dense Hermitian diagonalization is cheap at d=378 and avoids the
    # loss of orthogonality that eigsh can show inside an exactly/quasi
    # degenerate cat doublet.  A non-orthogonal frame corrupts leakage.
    ...
    W = V[:, pair]
    # Defensive orthonormalization of the selected invariant subspace.
    W, _ = np.linalg.qr(W)
